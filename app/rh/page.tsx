...
export default function RHPage() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        console.error("Erreur de liaison Supabase:", err);
      } finally {
        // Petit délai pour l'effet de scan fluide
  ...
  // ÉCRAN DE CHARGEMENT "SCANNER" (Version 100% Tailwind - Sans Erreur)
  if (loading) {
    return (
      <div className="flex h-screen bg-[#020504] items-center justify-center">
        <div className="flex flex-col items-center gap-6">
          ...
          <div className="text-center space-y-2">
            <p className="text-emerald-500 font-black tracking-[0.4em] text-[10px] uppercase">Cryptage de la liaison...</p>
      ...
      <main className="flex-1 p-4 flex flex-col gap-4 overflow-hidden relative">
        ...
        <div className="flex flex-col md:flex-row justify-between items-center gap-4 shrink-0">
          <div className="text-center md:text-left">
            ...
            <p className="text-[9px] text-gray-400 uppercase tracking-[0.3em] font-bold">Node RH • Ecodreum Burundi</p>
          </div>
        ...
        <div className="flex-1 overflow-y-auto pr-2 space-y-4">
          ...
          {view === 'members' ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4 pb-10">
              {staff.map((member) => (
                <div key={member.id} className="glass-card p-5 border-t-2 border-t-emerald-500/30 relative overflow-hidden group hover:scale-[1.01] transition-transform">
                  <div className="flex items-start justify-between mb-6">
                    <div className="w-14 h-14 rounded-2xl bg-white/5 border border-white/10 flex items-center justify-center text-2xl font-black text-emerald-500 italic">
                      {member.full_name?.charAt(0) || "E"}
                    </div>
                    <div className="text-right">
                       <p className="text-[8px] text-emerald-500 font-black uppercase tracking-widest">Performance</p>
                       <p className="text-xl font-black italic">{member.performance_score || 100}%</p>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-1 gap-2 border-t border-white/5 pt-4 mb-4 text-[10px] text-gray-400 font-medium">
                  ...
                  <button className="w-full py-2.5 bg-emerald-500/5 hover:bg-emerald-500/20 border border-emerald-500/20 rounded-xl text-[9px] font-black uppercase tracking-[0.2em] flex items-center justify-center transition-all">
                    Scanner Profil <ChevronRight size={12} className="ml-2" />
                  </button>
                </div>
              ))}
            </div>
            ...
            <div className="space-y-4 pb-20">
               <div className="glass-card p-6 flex flex-col md:flex-row justify-between items-center gap-4 border-l-4 border-l-gold bg-gold/5">
                  ...
                  <button className="w-full md:w-auto bg-gold text-black px-8 py-3 rounded-xl font-black text-[10px] uppercase tracking-widest flex items-center justify-center transition-all shadow-lg active:scale-95">
                    <Plus size={16} className="mr-2" /> Nouvelle Mission
                  </button>
               </div>

               <div className="glass-card p-5 flex flex-col md:flex-row justify-between items-center gap-4 border border-white/5">
                  <div className="flex items-center space-x-4">
                    <div className="w-12 h-12 bg-white/5 rounded-2xl flex items-center justify-center border border-white/10">
                      <Play size={18} className="text-emerald-500 ml-1" />
                    </div>
                    <div>
                      <h4 className="font-black text-sm uppercase tracking-tight">Analyse de Marché BUJUMBURA</h4>
                      <p className="text-[9px] text-gray-400 font-bold uppercase tracking-widest text-emerald-500">Statut: En cours</p>
                    </div>
                  </div>
                  <button className="px-6 py-2.5 bg-emerald-500 text-black rounded-xl text-[9px] font-black uppercase tracking-widest hover:bg-emerald-400 transition-all">
                    Rapport IA
                  </button>
               </div>
            </div>
        ...
        <button className="fixed bottom-6 right-6 bg-gradient-to-br from-emerald-500 to-gold p-4 rounded-2xl shadow-2xl flex items-center space-x-3 hover:scale-110 transition-all z-50">
           ...
           <div className="pr-2 text-black text-left">
              ...
              <p className="text-[10px] font-black uppercase tracking-tighter italic">Générer Rapport</p>
