...
const GoldBadge = ({ letter, name }: { letter: string, name: string }) => (
  <div className="relative z-10 flex flex-col items-center group">
    ...
    <div className="mt-6 glass-card px-6 py-2 border-t-2 border-t-gold/50 bg-black/60 rounded-xl text-center">
        ...
        <p className="text-[8px] text-white/40 uppercase font-bold tracking-[0.2em] mt-1 text-nowrap">Chef de Département (VIP)</p>
    </div>
  </div>
);
...
export default function RHPage() {
  ...
  // États pour la gestion des Missions
  ...
  const handleDeployTask = async () => {
    ...
    if (!error) {
        setShowModal(false);
        setNewTask({ title: '', assigned_to: '', priority: 'Normale', department: '' });
  ...
  return (
    <div className="flex h-screen bg-transparent text-white overflow-hidden font-sans">
      ...
      <main className="flex-1 p-4 flex flex-col gap-4 overflow-hidden relative">
            ...
            <>
              {!selectedDept ? (
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 animate-in fade-in slide-in-from-bottom-4 duration-500">
                  {DEPARTMENTS.map((dept) => (
                    <button key={dept} onClick={() => setSelectedDept(dept)} className="glass-card p-8 border border-white/5 hover:border-emerald-500/50 hover:bg-emerald-500/5 transition-all group flex flex-col items-center gap-3">
                      <div className="w-12 h-12 rounded-full border-2 border-emerald-500/30 flex items-center justify-center group-hover:scale-110 transition-transform shadow-[0_0_15px_rgba(16,185,129,0.1)]"><Users className="text-emerald-500" size={20} /></div>
                ...
                <div className="h-full flex flex-col min-h-[650px] relative animate-in zoom-in duration-300">
                  <button onClick={() => setSelectedDept(null)} className="flex items-center text-xs font-bold text-emerald-500 mb-4 z-50 hover:translate-x-[-4px] transition-transform"><ChevronLeft size={16} /> Retour aux secteurs</button>
                       ...
                       {staff.filter(m => m.department === selectedDept && !m.role.toLowerCase().includes('chef') && !m.role.toLowerCase().includes('ceo')).map((member, index, array) => {
                         ...
                         return (
                           <div key={member.id} style={{ transform: `translate(${x}px, ${y}px)` }} className="absolute flex flex-col items-center group cursor-pointer z-20">
                             ...
                             <div className="opacity-0 group-hover:opacity-100 transition-opacity absolute -bottom-12 bg-black/90 p-2 rounded border border-emerald-500/30 w-32 text-center pointer-events-none shadow-2xl">
            ...
            <div className="space-y-4 pb-20 animate-in fade-in duration-500">
               <div className="glass-card p-6 flex flex-col md:flex-row justify-between items-center gap-4 border-l-4 border-l-gold bg-gold/5">
                  <div>
                    ...
                    <p className="text-[9px] text-gray-400 uppercase tracking-widest font-bold">Déploiement en temps réel</p>
                  </div>
                  <button 
                    onClick={() => {
                        if (!selectedDept) alert("Veuillez d'abord sélectionner un département dans 'Collaborateurs'");
                        else setShowModal(true);
                    }}
                    className="bg-gold text-black px-8 py-3 rounded-xl font-black text-[10px] uppercase tracking-widest flex items-center shadow-lg hover:brightness-110 active:scale-95 transition-all"
                  >
                    <Plus size={16} className="mr-2" /> Injecter Mission
                  </button>
               </div>
               {/* Exemple de Tâche */}
               <div className="glass-card p-5 flex items-center justify-between border border-white/5 group hover:border-emerald-500/30 transition-colors">
                  <div className="flex items-center space-x-4">
                    ...
                    <div><h4 className="font-black text-sm uppercase italic">Audit de Performance {selectedDept}</h4><p className="text-[9px] text-emerald-500 font-black uppercase tracking-[0.2em] animate-pulse">Statut: Analyse IA en cours</p></div>
                  </div>
                  <button className="px-6 py-2 bg-emerald-500/20 text-emerald-400 border border-emerald-500/30 rounded-xl text-[9px] font-black uppercase hover:bg-emerald-500 hover:text-black transition-all">Consulter</button>
               </div>
            </div>
          )}
        </div>

        {/* MODAL D'INJECTION DE MISSION */}
        {showModal && (
          <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-black/80 backdrop-blur-xl">
            <div className="glass-card w-full max-w-md border-t-4 border-t-gold p-8 relative animate-in zoom-in duration-300">
              ...
              <h2 className="text-xl font-black italic text-gold uppercase mb-6 tracking-tighter">Injection Mission : {selectedDept}</h2>
              <div className="space-y-5">
                <div>
                  <label className="text-[10px] font-black uppercase text-gray-500 tracking-widest">Intitulé de la Cible</label>
                  <input type="text" placeholder="Nom de la mission..." className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-sm focus:border-gold outline-none transition-all mt-1" onChange={(e) => setNewTask({...newTask, title: e.target.value})} />
                </div>
                <div>
                  <label className="text-[10px] font-black uppercase text-gray-500 tracking-widest">Assigner au Spécialiste</label>
                  <select className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-sm focus:border-gold outline-none mt-1 text-white appearance-none" onChange={(e) => setNewTask({...newTask, assigned_to: e.target.value})}>
                    <option value="" className="bg-black">-- Choisir un agent --</option>
                ...
                <div className="flex gap-4 pt-4">
                  ...
                  <button onClick={handleDeployTask} disabled={isDeploying} className="flex-1 py-3 bg-gold text-black rounded-xl text-[10px] font-black uppercase shadow-lg shadow-gold/20 flex items-center justify-center">
                    {isDeploying ? <Loader2 className="animate-spin" size={16} /> : "Déployer Mission"}
                  </button>
                </div>
              </div>
            </div>
        ...
        {/* BOUTON FLOTTANT IA */}
        <button className="fixed bottom-6 right-6 bg-gradient-to-br from-emerald-500 to-gold p-4 rounded-2xl shadow-2xl flex items-center space-x-3 hover:scale-110 transition-all z-50 group">
           <div className="bg-black/20 p-2 rounded-lg group-hover:rotate-12 transition-transform"><CheckCircle size={20} className="text-black" /></div>
           <div className="pr-2 text-black text-left">
              <p className="text-[8px] font-black uppercase leading-none opacity-60">IA Scanner Core</p>
