               ...
               {view === 'members' ? (
                 ...
                 <div className="space-y-6">
                    <div className="flex flex-wrap items-center justify-between gap-4">
                        ...
                        {/* BOUTON AJOUTER MISSION - TEXTE VERT EMERAUDE */}
                        <button 
                          onClick={() => setShowTaskModal(true)} 
                          className="bg-black border-2 border-emerald-500/50 px-8 py-3 rounded-xl font-black text-[11px] uppercase flex items-center gap-2 shadow-[0_0_15px_rgba(16,185,129,0.2)] hover:bg-emerald-500/10 transition-all"
                        >
                            ...
                            <div key={task.id} className="glass-card p-4 border border-white/5 flex flex-col md:flex-row justify-between items-center gap-4">
                                ...
                                <div className="flex items-center gap-6">
                                    <div className="flex items-center gap-2 text-gray-400">
                                        ...
                                        <span className="text-[9px] font-black uppercase">Délai : {task.deadline}</span>
                                    </div>
                                </div>
                            </div>
                        ))}
        ...
        {showTaskModal && (
            <div className="fixed inset-0 z-[300] flex items-center justify-center bg-black/90 p-4 backdrop-blur-xl">
                <div className="glass-card w-full max-w-md p-8 border-t-4 border-t-emerald-500 animate-in zoom-in duration-300">
                    <div className="flex justify-between items-center mb-6">
                        <h2 className="text-xl font-black text-emerald-500 uppercase italic">Nouvelle Mission</h2>
                        <button onClick={() => setShowTaskModal(false)}><X className="text-white" /></button>
                    </div>
                    <div className="space-y-5">
                        <div>
                            <label className="text-[9px] font-black text-gray-500 uppercase">Intitulé</label>
                            <input type="text" className="w-full bg-white/5 border border-white/10 p-4 rounded-xl outline-none focus:border-emerald-500 text-white text-sm" placeholder="Nom de la tâche..." onChange={(e)=>setNewTask({...newTask, title: e.target.value})}/>
                        </div>
                        <div>
                            <label className="text-[9px] font-black text-gray-500 uppercase">Assigner à</label>
        ...
        {showPinModal && (
            <div className="fixed inset-0 z-[400] flex items-center justify-center bg-black/95 backdrop-blur-2xl">
                <div className="glass-card p-10 border border-emerald-500/20 flex flex-col items-center gap-6 animate-in zoom-in">
                    ...
                    <button onClick={handlePinSubmit} className="w-full py-3 bg-emerald-500 text-black text-[10px] font-black uppercase rounded-lg">Vérifier l'Accès</button>
                </div>
            </div>
        )}
      </main>
