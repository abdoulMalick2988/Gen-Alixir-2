...
export default function GenerateurContratFinal() {
  ...
  const generatePDF = async () => {
    ...
    try {
      ...
      const margin = 20; // Marges augmentées
      const contentWidth = pageWidth - (2 * margin);
      const maxContentHeight = pageHeight - (2 * margin) - 10; // Marge supplémentaire en bas
  ...
  // GÉNÉRATION WORD POUR IMPRESSION
  const generateWord = async () => {
    ...
    try {
      showNotif("Génération du document Word...", "s");
      ...
      showNotif("Erreur lors de la génération du document", "e");
    }
  };
  ...
  const generateHTMLContent = () => {
    ...
    if (data.jobType === 'CDI') {
      contractTypeText = 'indéterminée (CDI)';
      specificClauses = `<p>Le présent contrat est conclu pour les tâches suivantes : ${data.jobDescription}</p>`;
    } else if (data.jobType === 'CDD') {
      contractTypeText = 'déterminée (CDD)';
      specificClauses = `<p>Le présent contrat est conclu pour les besoins suivants : ${data.cddReason}. Il prendra fin le ${new Date(data.endDate).toLocaleDateString('fr-FR')}.</p>`;
    } else if (data.jobType === 'STAGE') {
      contractTypeText = 'CONVENTION DE STAGE';
      specificClauses = `<p>Le présent stage porte sur les missions suivantes : ${data.stageTasks}. Il se déroulera du ${new Date(data.startDate).toLocaleDateString('fr-FR')} au ${new Date(data.endDate).toLocaleDateString('fr-FR')}.</p>`;
    }

    const remunerationSection = data.jobType === 'STAGE' 
      ? `<p>Le stagiaire percevra une gratification mensuelle de <strong>${data.stageAllowance} ${config.currency}</strong>.</p>`
      : `<p>En contrepartie de l'exécution de ses fonctions, le Salarié percevra une rémunération mensuelle brute de <strong>${data.salary} ${config.currency}</strong>.</p>
         ${data.bonus ? `<p>En sus de cette rémunération de base, le Salarié pourra percevoir les primes et avantages suivants : ${data.bonus}.</p>` : ''}`;
...
<head>
  ...
  <style>
    ...
    .signature-box { border-bottom: 1px solid black; height: 80px; margin: 20px 0; }
...
<body>
  ...
  <h2 style="text-align: center;">RÉGIME : ${data.jobType}</h2>
  ...
  immatriculée au RCCM sous le numéro <strong>${data.compRCCM}</strong> et identifiée au ${config.idLabel} sous le numéro <strong>${data.compID}</strong>, 
  représentée par M./Mme <strong>${data.bossName}</strong> en sa qualité de <strong>${data.bossTitle}</strong>.</p>
  ...
  <p>M./Mme <strong>${data.empName}</strong>, né(e) le <strong>${new Date(data.empBirth).toLocaleDateString('fr-FR')}</strong> à <strong>${data.empBirthPlace}</strong>, 
  ...
  <p style="text-align: right; font-style: italic;">Ci-après dénommé(e) « <strong>LE ${data.jobType === 'STAGE' ? 'STAGIAIRE' : 'SALARIÉ'}</strong> »</p>
  
  <p style="text-align: center;"><strong>D'AUTRE PART,</strong></p>
  
  <p style="text-align: center; margin-top: 30px;"><strong>IL A ÉTÉ ARRÊTÉ ET CONVENU CE QUI SUIT :</strong></p>
  
  <h2>ARTICLE 1 : OBJET ET CADRE LÉGAL</h2>
  <p>Le présent contrat est conclu sous le régime du ${config.code}.</p>
  <p>${config.articles.intro}</p>
  <p>${config.articles.engagement}</p>
  
  <h2>ARTICLE 2 : NATURE ET FONCTIONS</h2>
  <p>Le ${data.jobType === 'STAGE' ? 'Stagiaire' : 'Salarié'} est recruté en qualité de <strong>${data.jobTitle}</strong> au sein du département <strong>${data.jobDept}</strong>.</p>
  <p>Le ${data.jobType === 'STAGE' ? 'Stagiaire' : 'Salarié'} exercera ses fonctions au sein de l'établissement situé à <strong>${data.jobLocation}</strong>.</p>
  <p>Le type de contrat conclu est un contrat à durée <strong>${contractTypeText}</strong>.</p>
  ${specificClauses}
  
  <h2>ARTICLE 3 : RÉMUNÉRATION</h2>
  ${remunerationSection}
  <p>${config.articles.workDuration} la durée hebdomadaire de travail est fixée à <strong>${data.hours} heures</strong>.</p>
  
  <h2>ARTICLE 4 : DURÉE DU CONTRAT ${data.jobType !== 'STAGE' ? 'ET PÉRIODE D\'ESSAI' : ''}</h2>
  <p>Le présent contrat prend effet à compter du <strong>${new Date(data.startDate).toLocaleDateString('fr-FR')}</strong>${data.endDate ? ` et prendra fin le ${new Date(data.endDate).toLocaleDateString('fr-FR')}` : ''}.</p>
  ${data.jobType !== 'STAGE' ? `<p>Une période d'essai de <strong>${data.trial} mois</strong> est prévue.</p>` : ''}
  ...
  - Respecter l'ensemble des dispositions légales applicables</p>
  
  <p><strong>Le ${data.jobType === 'STAGE' ? 'Stagiaire' : 'Salarié'} s'engage à :</strong><br>
  - Exécuter personnellement les missions confiées<br>
  - Respecter les directives de l'Employeur<br>
  - Observer une obligation de loyauté et de confidentialité</p>
  
  ${data.hasNonCompete && data.jobType !== 'STAGE' ? `
  <h2>ARTICLE 6 : CLAUSE DE NON-CONCURRENCE</h2>
  <p>Le Salarié s'engage, pendant une durée de <strong>${data.nonCompeteDuration}</strong> suivant la cessation du présent contrat, 
  à ne pas exercer une activité concurrente à celle de l'Employeur.</p>
  ` : ''}
  
  <h2>ARTICLE ${data.hasNonCompete && data.jobType !== 'STAGE' ? '7' : '6'} : LITIGES</h2>
  <p>En cas de différend, tout litige relèvera de la compétence exclusive du <strong>${config.court}</strong>.</p>
  
  <div class="signature-section">
    <p>Fait à <strong>${data.compAddr.split(',')[0].trim()}</strong>, le <strong>${new Date().toLocaleDateString('fr-FR', { day: 'numeric', month: 'long', year: 'numeric' })}</strong></p>
    ...
    <table>
      <tr>
        <td style="text-align: center;">
          ...
          <p><strong>${data.bossName}</strong><br>${data.bossTitle}</p>
        </td>
        <td style="text-align: center;">
          <p><strong>LE ${data.jobType === 'STAGE' ? 'STAGIAIRE' : 'SALARIÉ'}</strong></p>
          <div class="signature-box"></div>
          <p><strong>${data.empName}</strong><br>${data.jobTitle}</p>
        </td>
      </tr>
    </table>
  </div>
  
  <p style="text-align: center; margin-top: 40px; font-size: 10pt; color: #666;">
    ${data.documentMode === 'PRINT' ? data.compName : 'Document généré via ECODREUM Intelligence'}
  </p>
</body>
</html>
    `;
      ...
      <div className="max-w-6xl mx-auto p-3 md:p-6 relative" style={{ zIndex: 10 }}>
                        ...
                        <div className="relative z-10 space-y-5">
                          ...
                          {data.jobType === 'CDI' && (
                            <div className="bg-emerald-500/10 border border-emerald-400/30 rounded-xl p-4">
                              <InputField label="Tâches Confiées" value={data.jobDescription} onChange={(v) => updateData('jobDescription', v)} icon={<FileText size={12} />} required multiline />
                            </div>
                          )}
                          ...
                          {data.jobType === 'STAGE' && (
                            <div className="bg-purple-500/10 border border-purple-400/30 rounded-xl p-4">
                              <InputField label="Missions du Stage" value={data.stageTasks} onChange={(v) => updateData('stageTasks', v)} icon={<FileText size={12} />} required multiline />
                            </div>
                          )}

                          {/* Rémunération selon type */}
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            {data.jobType === 'STAGE' ? (
                              <InputField label={`Indemnité de Stage (${config.currency})`} type="number" value={data.stageAllowance} onChange={(v) => updateData('stageAllowance', v)} icon={<DollarSign size={12} />} required />
                            ) : (
                            ...
                            <InputField label="Primes / Avantages" value={data.bonus} onChange={(v) => updateData('bonus', v)} icon={<Award size={12} />} />
                          </div>
        ...
        <div className="glass-card rounded-2xl p-5 sticky top-4 space-y-4 mb-8">
          ...
          <div className="relative z-10 space-y-4">
            ...
            {/* SIGNATURES (uniquement en mode électronique) */}
            {data.documentMode === 'ELECTRONIC' && (
            ...
            <div className="space-y-3 pt-4 border-t border-cyan-400/20">
              {data.documentMode === 'ELECTRONIC' ? (
                ...
                <button
                  ...
                  className="w-full py-4 bg-gradient-to-r from-yellow-500 to-orange-500 hover:from-yellow-400 hover:to-orange-400 text-black rounded-xl font-black uppercase text-sm flex items-center justify-center gap-2 transition-all shadow-lg shadow-yellow-500/50 hover:shadow-yellow-500/70 hover:scale-105"
                >
                  <Download size={18} />
          ...
          <div className="fixed inset-0 bg-black/95 backdrop-blur-sm z-[9998] overflow-y-auto">
            ...
            <div className="min-h-screen flex items-center justify-center p-4 md:p-8 pt-0">
              <div 
                ...
                className="bg-white text-black w-[210mm] min-h-[297mm] shadow-2xl"
                style={{ padding: '20mm' }} // Marges internes augmentées
              >
...
function ContractPreview({ data, config, signatures }: { data: FormData; config: CountryConfig; signatures: { employer: string; employee: string } }) {
  ...
  if (data.jobType === 'CDI') {
    contractTypeText = 'indéterminée (CDI)';
    specificClauses = data.jobDescription ? `\n\nLe présent contrat est conclu pour les tâches suivantes : ${data.jobDescription}.` : '';
  } else if (data.jobType === 'CDD') {
    contractTypeText = 'déterminée (CDD)';
    specificClauses = data.cddReason ? `\n\nLe présent contrat est conclu pour les besoins suivants : ${data.cddReason}.` : '';
  } else if (data.jobType === 'STAGE') {
    contractTypeText = 'CONVENTION DE STAGE';
    specificClauses = data.stageTasks ? `\n\nLe présent stage porte sur les missions suivantes : ${data.stageTasks}.` : '';
  }

  const bonusClause = data.bonus ? `\n\nEn sus de cette rémunération, le ${data.jobType === 'STAGE' ? 'Stagiaire' : 'Salarié'} pourra percevoir les avantages suivants : ${data.bonus}.` : '';
  ...
  return (
    <div className="space-y-6 font-serif" style={{ fontFamily: 'Georgia, serif', lineHeight: '1.8' }}>
      ...
      <div className="page-break-avoid space-y-6 text-sm">
        ...
        <p className="italic text-right">Ci-après dénommé(e) « <strong>LE {data.jobType === 'STAGE' ? 'STAGIAIRE' : 'SALARIÉ'}</strong> »</p>
      ...
      <div className="page-break-avoid">
        <Article title="ARTICLE 2 : NATURE ET FONCTIONS">
          Le {data.jobType === 'STAGE' ? 'Stagiaire' : 'Salarié'} est recruté en qualité de <strong>{data.jobTitle}</strong> au sein du département <strong>{data.jobDept}</strong>.
          <br /><br />
          Le {data.jobType === 'STAGE' ? 'Stagiaire' : 'Salarié'} exercera ses fonctions au sein de l'établissement situé à <strong>{data.jobLocation}</strong>.
          <br /><br />
          Le type de contrat conclu est un contrat à durée <strong>{contractTypeText}</strong>.{specificClauses}
          <br /><br />
          <strong>Le {data.jobType === 'STAGE' ? 'Stagiaire' : 'Salarié'} s'engage à</strong> exercer ses fonctions avec diligence, compétence et loyauté, conformément aux directives de l'Employeur et aux usages de la profession.
        </Article>
      </div>
      ...
      <div className="page-break-avoid">
        <Article title="ARTICLE 3 : RÉMUNÉRATION">
          {data.jobType === 'STAGE' ? (
            <>
              Le Stagiaire percevra une gratification mensuelle de <strong>{data.stageAllowance} {config.currency}</strong>.
            </>
          ) : (
            <>
      ...
      <div className="page-break-avoid">
        <Article title={`ARTICLE 4 : DURÉE DU CONTRAT${data.jobType !== 'STAGE' ? ' ET PÉRIODE D\'ESSAI' : ''}`}>
          Le présent contrat prend effet à compter du <strong>{new Date(data.startDate).toLocaleDateString('fr-FR')}</strong>{endDateClause}.
          {data.jobType !== 'STAGE' && (
            <>
              <br /><br />
      ...
      <div className="page-break-avoid">
        <Article title="ARTICLE 5 : OBLIGATIONS DES PARTIES">
          ...
          <br />
          - Fournir au {data.jobType === 'STAGE' ? 'Stagiaire' : 'Salarié'} un travail conforme à ses qualifications professionnelles
          <br />
          ...
          <br /><br />
          <strong>Le {data.jobType === 'STAGE' ? 'Stagiaire' : 'Salarié'} s'engage à :</strong>
          <br />
      ...
      <div className="page-break-avoid">
        <Article title={`ARTICLE ${data.hasNonCompete && data.jobType !== 'STAGE' ? '7' : '6'} : ${data.jobType === 'STAGE' ? 'FIN DU STAGE' : 'SUSPENSION ET RUPTURE DU CONTRAT'}`}>
          {data.jobType === 'STAGE' ? (
            <>
              Le présent stage prendra fin à la date prévue sans nécessité de préavis.
              <br /><br />
              En cas de manquement grave aux obligations du Stagiaire, l'Employeur se réserve le droit de mettre fin au stage de manière anticipée.
            </>
          ) : (
            <>
      ...
      <div className="page-break-avoid">
        <Article title={`ARTICLE ${data.hasNonCompete && data.jobType !== 'STAGE' ? '8' : '7'} : LITIGES`}>
      ...
      {/* SIGNATURES */}
      <div className="page-break-avoid" style={{ marginTop: '40px' }}>
        <div className="space-y-8">
          ...
          <p className="text-sm">En deux exemplaires originaux, dont un remis au {data.jobType === 'STAGE' ? 'Stagiaire' : 'Salarié'}.</p>
          ...
          <div className="grid grid-cols-2 gap-8 mt-12">
            ...
            <div className="text-center space-y-4">
              <p className="font-bold">LE {data.jobType === 'STAGE' ? 'STAGIAIRE' : 'SALARIÉ'}</p>
