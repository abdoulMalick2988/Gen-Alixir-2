...
function buildPDF(data: FormData, config: CountryConfig, signatures: { employer: string; employee: string }): jsPDF {
  ...
  const PW = 210, PH = 297, ML = 22, MR = 22, MT = 20, MB = 20;
  const CW = PW - ML - MR;
  let y = MT;
  let page = 1;
  ...
  const nl = (size: number) => size * 0.45 + 0.8;
  ...
  const addText = (
    ...
    const maxW = align === 'center' ? CW : CW - 2;
    const lines = pdf.splitTextToSize(text, maxW);
    const lineH = nl(size);
    checkY(lines.length * lineH + 2);
    const x = align === 'center' ? PW / 2 : align === 'right' ? PW - MR - 1 : ML;
    pdf.text(lines, x, y, { align, maxWidth: maxW });
    y += lines.length * lineH;
    return lines.length;
  };

  const addBoldValue = (label: string, value: string, size: number = 9.5, color: [number, number, number] = [30, 30, 30]) => {
    pdf.setFont('helvetica', 'normal');
    pdf.setFontSize(size);
    pdf.setTextColor(...color);
    const fullText = label + value;
    const lines = pdf.splitTextToSize(fullText, CW - 2);
    const lineH = nl(size);
    checkY(lines.length * lineH + 2);
    pdf.text(lines, ML, y, { maxWidth: CW - 2 });
    y += lines.length * lineH;
  };
  ...
  const addArticle = (title: string, body: string) => {
    ...
    pdf.setFont('helvetica', 'bold');
    pdf.setFontSize(10);
    const titleLines = pdf.splitTextToSize(title, CW - 8);
    const lineH = nl(10);
    const titleBlockH = titleLines.length * lineH;
    checkY(titleBlockH + 4);
    pdf.setFillColor(0, 100, 170);
    pdf.rect(ML, y, 2.5, titleBlockH, 'F');
    pdf.setTextColor(0, 80, 140);
    pdf.text(titleLines, ML + 6, y + lineH * 0.7, { maxWidth: CW - 8 });
    y += titleBlockH + 2;
    addSpacer(1.5);
    ...
    for (const line of bodyLines) {
      ...
      if (trimmed.startsWith('—')) {
        ...
        const splitLines = pdf.splitTextToSize(trimmed, CW - 10);
        const lH = nl(9);
        checkY(splitLines.length * lH + 1);
        pdf.text(splitLines, ML + 6, y, { maxWidth: CW - 10 });
        y += splitLines.length * lH;
      } else {
        pdf.setFont('helvetica', 'normal');
        pdf.setFontSize(9);
        pdf.setTextColor(40, 40, 40);
        const splitLines = pdf.splitTextToSize(trimmed, CW - 2);
        const lH = nl(9);
        checkY(splitLines.length * lH + 1);
        pdf.text(splitLines, ML, y, { maxWidth: CW - 2 });
        y += splitLines.length * lH;
      }
      addSpacer(0.8);
    }
  ...
  if (data.compDescription) {
    ...
    const descLines = pdf.splitTextToSize(data.compDescription, CW - logoOffset - 2);
  ...
  addBoldValue('', `M./Mme ${data.empName}, né(e) le ${data.empBirth ? new Date(data.empBirth).toLocaleDateString('fr-FR') : '—'} à ${data.empBirthPlace}, de nationalité ${data.empNation}${foreignerClause}, titulaire de la pièce d'identité n°${data.empID}, demeurant à ${data.empAddr}, joignable au ${data.empPhone}.`);
  addSpacer(2);
...
function buildWordDocument(data: FormData, config: CountryConfig): string {
  ...
  const capitalClause = data.showCapital && data.compCapital ? `, au capital social de ${new Intl.NumberFormat('fr-FR').format(parseFloat(data.compCapital) || 0)} ${config.currency}` : '';
  const foreignerClause = data.isForeigner && data.empWorkPermit ? `, titulaire du permis de travail n&deg;${data.empWorkPermit}` : '';
  ...
  let specificClause = '';
  if (data.jobType === 'CDI' && data.jobDescription) specificClause = `<p class="body"><b>T&acirc;ches confi&eacute;es :</b> ${esc(data.jobDescription)}.</p>`;
  if (data.jobType === 'CDD' && data.cddReason) specificClause = `<p class="body"><b>Motif du CDD :</b> ${esc(data.cddReason)}.</p>`;
  if (data.jobType === 'STAGE' && data.stageTasks) specificClause = `<p class="body"><b>Missions du stage :</b> ${esc(data.stageTasks)}.</p>`;
  ...
  const bonusHtml = data.bonus ? `<p class="body"><b>Avantages compl&eacute;mentaires :</b> ${esc(data.bonus)}.</p>` : '';
...
export default function GenerateurContrat() {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!allowedTypes.includes(file.type)) { showNotif('Seuls PDF et Word acceptés', 'e'); return; }
    setIsUploading(true);
    try {
      ...
      showNotif('Fichier archivé avec succès', 's');
      if (fileInputRef.current) fileInputRef.current.value = '';
    } catch (err) { console.error('Erreur upload:', err); showNotif('Erreur lors du chargement', 'e'); }
  ...
  const downloadArchivedFile = async (contract: SavedContract) => {
    ...
    try {
      ...
      const fileName = `CONTRAT_${(contract.employeeName || 'document').replace(/\s+/g, '_')}${ext}`;
    ...
    } catch (err) { console.error('Erreur:', err); showNotif('Erreur de téléchargement', 'e'); }
  };
  ...
  const regenerateAndDownload = (contract: SavedContract) => {
    ...
    try {
      ...
      if (contract.mode === 'ELECTRONIC' || contract.fileType === 'PDF') {
        ...
        pdf.save(`CONTRAT_${(contract.employeeName || 'document').replace(/\s+/g, '_')}.pdf`);
      } else {
        const html = buildWordDocument(contract.data, cConfig);
        const blob = new Blob(['\ufeff', html], { type: 'application/msword' });
        const url = URL.createObjectURL(blob);
        const a = document.createElement('a'); a.href = url;
        a.download = `CONTRAT_${(contract.employeeName || 'document').replace(/\s+/g, '_')}.doc`;
        document.body.appendChild(a); a.click(); document.body.removeChild(a); URL.revokeObjectURL(url);
      }
      showNotif('Document téléchargé', 's');
    } catch (err) { console.error('Erreur:', err); showNotif('Erreur de génération', 'e'); }
  };
  ...
  const generatePDF = async () => {
    if (!validateForm()) { showNotif('Remplissez tous les champs obligatoires', 'e'); return; }
    ...
    } catch (err) { console.error(err); showNotif('Erreur de génération', 'e'); }
  ...
  const generateWord = async () => {
    ...
    try {
      ...
      showNotif('Document Word généré !', 's');
    } catch (err) { console.error(err); showNotif('Erreur de génération', 'e'); }
  };
                        ...
                        <div key={c.id} className="glass rounded-xl p-4 space-y-3 glass-hover">
                          ...
                          <div className="relative z-10">
                            ...
                            <div className="flex gap-2 pt-3">
                              ...
                              <button onClick={() => c.fileUrl ? downloadArchivedFile(c) : regenerateAndDownload(c)} className="flex-1 py-2 rounded-lg text-[11px] font-bold flex items-center justify-center gap-1.5" style={{ background: 'rgba(0,229,255,0.12)', border: '1px solid rgba(0,229,255,0.3)', color: '#67e8f9' }}><Download size={11} /> Télécharger</button>
              ...
              <div className="relative z-10 flex-1 overflow-auto scrollable-container p-5 space-y-4">
                {previewContract.data && (
                  <div className="gold-card rounded-xl p-4 space-y-2">
                    ...
                    {previewContract.signed && (<div className="flex items-center gap-1.5 text-emerald-400 font-bold pt-2 text-xs"><CheckCircle size={13} /> Signé électroniquement</div>)}
                  </div>
                )}
                <div className="rounded-xl p-4 text-center" style={{ background: 'rgba(0,229,255,0.06)', border: '1px solid rgba(0,229,255,0.2)' }}>
                  ...
                  <button onClick={() => previewContract.fileUrl ? downloadArchivedFile(previewContract) : regenerateAndDownload(previewContract)}
