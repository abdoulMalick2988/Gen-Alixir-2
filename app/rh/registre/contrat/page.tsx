...
export default function GenerateurContrat() {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!allowedTypes.includes(file.type)) {
      showNotif('Seuls les fichiers PDF et Word sont acceptés', 'e');
      return;
    }
    ...
    try {
      ...
      showNotif('Erreur lors du chargement du fichier', 'e');
    } finally {
  ...
  const generateWord = async () => {
    ...
    try {
      ...
      showNotif('Document Word généré et archivé avec succès !', 's');
    } catch (err) {
      console.error(err);
        ...
        <div className="glass rounded-2xl p-5 mb-8">
          ...
          <div className="relative z-10 space-y-4">
            ...
            <div className="space-y-2">
              <h4 className="text-[11px] font-black uppercase text-cyan-400 flex items-center gap-1.5"><Upload size={11} /> Charger un Contrat</h4>
              <input
                ref={fileInputRef}
                type="file"
                accept=".pdf,.doc,.docx"
                          ...
                          <div className="relative z-10">
                            <div className="flex justify-between items-start mb-2">
                              ...
                              <span className="text-[10px] px-2 py-0.5 rounded-lg font-bold" style={c.mode === 'ELECTRONIC' ? { background: 'rgba(0,229,255,0.12)', color: '#67e8f9' } : { background: 'rgba(245,158,11,0.12)', color: '#fcd34d' }}>
                                {c.fileType || (c.mode === 'ELECTRONIC' ? 'PDF' : 'WORD')}
                              </span>
                            </div>
                            ...
                            <div className="flex gap-2 pt-3">
                              <button 
                                ...
                                className="flex-1 py-2 rounded-lg text-[11px] font-bold flex items-center justify-center gap-1.5 transition-colors" 
                                style={{ background: 'rgba(168,85,247,0.12)', border: '1px solid rgba(168,85,247,0.3)', color: '#c084fc' }}
                              >
                                <Eye size={11} /> Aperçu
                              </button>
            ...
            <div className="glass rounded-2xl w-full max-w-4xl max-h-[90vh] flex flex-col fade-in">
              ...
              <div className="relative z-10 p-5 border-b" style={{ borderColor: 'rgba(0,229,255,0.15)' }}>
                <div className="flex justify-between items-center">
                  <h2 className="text-xl font-black uppercase text-cyan-300 flex items-center gap-2">
                    <Eye size={20} className="text-purple-400" /> Aperçu du Contrat
                  </h2>
                  <button onClick={() => setPreviewContract(null)} className="p-2 rounded-xl transition-colors" style={{ background: 'rgba(239,68,68,0.15)', border: '1px solid rgba(239,68,68,0.4)', color: '#f87171' }}>
                    <X size={18} />
                  </button>
                </div>
                <div className="mt-3 flex items-center gap-2 text-xs text-cyan-300/70">
                  <span className="font-bold">{previewContract.employeeName}</span>
                  <span>•</span>
                  <span>{previewContract.jobTitle}</span>
                  <span>•</span>
                  <span>{new Date(previewContract.createdAt).toLocaleDateString('fr-FR')}</span>
                </div>
              </div>
              <div className="relative z-10 flex-1 overflow-auto scrollable-container p-5">
                {previewContract.fileUrl ? (
                  previewContract.fileType === 'PDF' ? (
                    <iframe 
                    ...
                    <div className="space-y-4">
                      <div className="rounded-xl p-4 text-center" style={{ background: 'rgba(245,158,11,0.08)', border: '1px solid rgba(245,158,11,0.25)' }}>
                        ...
                        <p className="text-amber-300 font-bold mb-2">Document Word</p>
                        <p className="text-xs text-amber-300/70 mb-4">L'aperçu des fichiers Word n'est pas disponible dans le navigateur</p>
                        <a 
                          href={previewContract.fileUrl} 
                          download 
                      ...
                      {previewContract.data && (
                        <div className="gold-card rounded-xl p-4 space-y-2">
                          ...
                          {previewContract.data.jobType !== 'STAGE' && (
                            <InfoRow label="Salaire" value={`${previewContract.data.salary} ${COUNTRIES[previewContract.data.country].currency}`} />
                          )}
