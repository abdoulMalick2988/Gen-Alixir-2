...
export default function GenerateurContratFinal() {
      ...
      const doc = new Document({
        ...
        sections: [{
          ...
          children: [
            ...
            new Paragraph({
              children: [new TextRun({ text: "L'EMPLOYEUR (Signature & Cachet)\t\t\t\tLE SALARIÉ (Lu et approuvé)", bold: true })],
              spacing: { before: 400 }
            })
          ]
        }]
  ...
  const saveToCloud = async () => {
    ...
    try {
      ...
      showNotif("Enregistré !", "s");
    } catch (e) {
      showNotif("Archivé localement", "s");
    } finally {
      setIsSaving(false);
    }
  };
  ...
  return (
    <div className="fixed inset-0 bg-[#050505] text-white overflow-y-auto selection:bg-emerald-500/30">
      <div className="max-w-6xl mx-auto p-4 md:p-8">
        ...
        {notif && (
          <div className={`fixed top-6 left-1/2 -translate-x-1/2 z-50 px-6 py-3 rounded-2xl border backdrop-blur-xl ${notif.t === 's' ? 'bg-emerald-500/10 border-emerald-500/20 text-emerald-400' : 'bg-red-500/10 border-red-500/20 text-red-400'}`}>
        ...
        <div className="flex flex-col md:flex-row justify-between items-center gap-6 mb-12">
          <div className="flex items-center gap-5">
            ...
            <div>
              ...
              <p className="text-[10px] font-bold text-zinc-500 uppercase tracking-widest">ECODREUM • V3.0.1</p>
            </div>
          </div>
          <div className="flex bg-zinc-900/50 p-1.5 rounded-2xl border border-white/5">
            {['SENEGAL', 'BURUNDI'].map((c) => (
              <button key={c} onClick={() => setActiveCountry(c as any)} className={`px-6 py-2 rounded-xl text-[10px] font-black transition-all ${activeCountry === c ? 'bg-emerald-500 text-black shadow-lg' : 'text-zinc-500 hover:text-white'}`}>{c}</button>
            ))}
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 pb-40">
          <div className="lg:col-span-8 space-y-6">
            <div className="bg-zinc-900/30 border border-white/5 p-6 rounded-[2.5rem] space-y-4">
              <div className="flex items-center gap-3 text-emerald-500 mb-2"><Building size={18} /><h2 className="text-xs font-black uppercase">Structure Employeuse</h2></div>
              <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                <InputMini label="Raison Sociale" val={data.compName} onChange={(v: string) => setData({...data, compName: v})} />
                <InputMini label="Type" val={data.compType} onChange={(v: string) => setData({...data, compType: v})} />
                <InputMini label={config.idLabel} val={data.compID} onChange={(v: string) => setData({...data, compID: v})} />
                <InputMini label="RCCM" val={data.compRCCM} onChange={(v: string) => setData({...data, compRCCM: v})} />
                <InputMini label="Signataire" val={data.bossName} onChange={(v: string) => setData({...data, bossName: v})} />
                <InputMini label="Fonction" val={data.bossTitle} onChange={(v: string) => setData({...data, bossTitle: v})} />
              </div>
            </div>

            <div className="bg-zinc-900/30 border border-white/5 p-6 rounded-[2.5rem] space-y-4">
              <div className="flex items-center gap-3 text-blue-400 mb-2"><User size={18} /><h2 className="text-xs font-black uppercase">Informations Salarié</h2></div>
              <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                <InputMini label="Nom Complet" val={data.empName} onChange={(v: string) => setData({...data, empName: v})} />
                <InputMini label="Né le" val={data.empBirth} onChange={(v: string) => setData({...data, empBirth: v})} />
                <InputMini label="Nationalité" val={data.empNation} onChange={(v: string) => setData({...data, empNation: v})} />
                <InputMini label="N° ID" val={data.empID} onChange={(v: string) => setData({...data, empID: v})} />
                <InputMini label="Téléphone" val={data.empPhone} onChange={(v: string) => setData({...data, empPhone: v})} />
                <InputMini label="Adresse" val={data.empAddr} onChange={(v: string) => setData({...data, empAddr: v})} />
              </div>
            </div>

            <div className="bg-zinc-900/30 border border-white/5 p-6 rounded-[2.5rem] space-y-4">
              <div className="flex items-center gap-3 text-amber-500 mb-2"><Briefcase size={18} /><h2 className="text-xs font-black uppercase">Conditions de Travail</h2></div>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                <div className="flex flex-col gap-1">
                  <label className="text-[9px] font-bold text-zinc-500 uppercase ml-1">Type</label>
                  <select value={data.jobType} onChange={e => setData({...data, jobType: e.target.value})} className="bg-black/40 border border-white/10 rounded-xl p-2.5 text-xs outline-none focus:border-amber-500 appearance-none">
                    <option value="CDI">CDI</option><option value="CDD">CDD</option><option value="STAGE">STAGE</option>
                  </select>
                </div>
                <InputMini label="Poste" val={data.jobTitle} onChange={(v: string) => setData({...data, jobTitle: v})} />
                <InputMini label="Salaire" type="number" val={data.salary} onChange={(v: string) => setData({...data, salary: v})} />
                <InputMini label="Début" type="date" val={data.startDate} onChange={(v: string) => setData({...data, startDate: v})} />
              </div>
            </div>
          </div>
          ...
          <div className="lg:col-span-4">
            <div className="bg-zinc-900 border border-white/10 p-8 rounded-[3rem] sticky top-8 shadow-2xl">
              ...
              <div className="space-y-4">
                <button onClick={generateWord} disabled={!data.empName || !data.jobTitle} className="w-full py-5 bg-emerald-500 hover:bg-emerald-400 disabled:bg-zinc-800 disabled:text-zinc-600 text-black rounded-2xl font-black uppercase text-xs flex items-center justify-center gap-3 transition-all active:scale-95 group">
                  <Download size={20} className="group-hover:animate-bounce" />Générer Word
                </button>
                <button onClick={saveToCloud} disabled={isSaving} className="w-full py-5 bg-white/5 hover:bg-white/10 border border-white/5 text-white rounded-2xl font-black uppercase text-xs flex items-center justify-center gap-3 transition-all">
                  {isSaving ? <span className="animate-spin text-lg">⏳</span> : <Save size={20} />}Archiver RH
                </button>
              </div>
              <div className="mt-10 p-5 bg-black/40 rounded-2xl border border-white/5 space-y-3">
                <div className="flex items-center gap-2 text-zinc-500"><ShieldCheck size={14} /><span className="text-[9px] font-black uppercase tracking-widest">Récapitulatif Légal</span></div>
                <div className="flex justify-between border-b border-white/5 pb-2"><span className="text-[9px] text-zinc-600 font-bold uppercase">Juridiction</span><span className="text-[9px] text-emerald-500 font-bold">{config.court}</span></div>
                <div className="flex justify-between"><span className="text-[9px] text-zinc-600 font-bold uppercase">Devise</span><span className="text-[9px] text-emerald-500 font-bold">{config.currency}</span></div>
              </div>
            </div>
          </div>
        </div>
...
// TYPAGE STRICT POUR LE COMPOSANT ENFANT
interface MiniProps {
  label: string;
  val: string;
...
function InputMini({ label, val, onChange, type = "text" }: MiniProps) {
  return (
    <div className="flex flex-col gap-1">
