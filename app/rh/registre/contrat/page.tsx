...
export default function GenerateurContratFinal() {
  ...
  useEffect(() => {
    if (activeSection && formSectionRef.current) {
      setTimeout(() => {
  ...
  // AMÉLIORATION : Génération PDF avec pagination intelligente
  const generatePDF = async () => {
    ...
    try {
      ...
      // Génération du PDF avec meilleure gestion des pages
      const pdf = new jsPDF({
      ...
      // Capturer chaque section séparément pour éviter les coupures
      const sections = contractRef.current.querySelectorAll('.pdf-section');
      ...
      if (sections.length > 0) {
        ...
        for (let i = 0; i < sections.length; i++) {
          ...
          // Si la section ne rentre pas, créer une nouvelle page
          if (currentY + imgHeight > pageHeight - margin && pageNumber > 0) {
            pdf.addPage();
            currentY = margin;
            pageNumber++;
          }
          ...
          currentY += imgHeight + 5; // 5mm d'espacement entre sections
        ...
        // Fallback : capture complète comme avant
  ...
  return (
    <div className="min-h-screen scroll-container" style={{
      ...
      <div className="max-w-6xl mx-auto p-3 md:p-6 relative" style={{ zIndex: 10 }}>
        ...
        {/* FORMULAIRE ACTIF avec scroll automatique */}
        {activeSection && (
          <div ref={formSectionRef} className="grid grid-cols-1 lg:grid-cols-12 gap-4 mb-20">
            {/* FORMULAIRE */}
            <div className="lg:col-span-8">
              
              {/* SECTION ENTREPRISE */}
              {activeSection === 'company' && (
                <div className="glass-card rounded-2xl p-5 space-y-5 slide-in">
                  <LuxuryPattern />
                  <div className="relative z-10 space-y-5">
                    <div className="flex items-center gap-3 pb-4 border-b border-cyan-400/20">
                      <div className="p-2 bg-gradient-to-br from-emerald-500 to-teal-500 rounded-xl shadow-lg shadow-emerald-500/50">
              ...
              {activeSection === 'contract' && (
                <div className="glass-card rounded-2xl p-5 space-y-5 slide-in">
                  ...
                  <div className="relative z-10 space-y-5">
                    <div className="flex items-center gap-3 pb-4 border-b border-cyan-400/20">
                      <div className="p-2 bg-gradient-to-br from-amber-500 to-yellow-500 rounded-xl shadow-lg shadow-amber-500/50">
...
function ContractPreview({ data, config, signatures }: { data: FormData; config: CountryConfig; signatures: { employer: string; employee: string } }) {
  ...
  return (
    <div className="space-y-6 font-serif" style={{ fontFamily: 'Georgia, serif', lineHeight: '1.8' }}>
      ...
      {/* PARTIES - Section séparée */}
      <div className="pdf-section page-break-avoid space-y-6 text-sm">
      ...
      {/* ARTICLES - Chaque article est une section séparée */}
      <div className="pdf-section page-break-avoid">
