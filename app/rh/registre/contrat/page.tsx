...
export default function GenerateurContratFinal() {
  ...
  const loadContractsFromSupabase = async () => {
    ...
    try {
      ...
      if (contracts && contracts.length > 0) {
        ...
        // Essayer de charger depuis localStorage si Supabase est vide
        loadFromLocalStorage();
      }
    } catch (error) {
  ...
  const saveContractToArchive = async (contractData: FormData, signed: boolean = false) => {
    ...
    const savedToSupabase = await saveContractToSupabase(contractData, verificationId, qrCode, signed);

    // Créer l'objet contrat local
    ...
    if (savedToSupabase) {
      showNotif('Contrat sauvegardé et synchronisé', 's');
    } else {
      showNotif('Contrat sauvegardé localement', 'w');
    }
  ...
  const deleteContract = async (id: string) => {
    ...
    setSavedContracts(updated);
    localStorage.setItem('ecodreum_contracts', JSON.stringify(updated));
    showNotif('Contrat supprimé définitivement', 's');
  };

  const loadContract = (contract: SavedContract) => {
    // Les contrats ne peuvent pas être modifiés, seulement visualisés
    showNotif('Les contrats ECODREUM ne peuvent pas être modifiés. Supprimez et recréez si nécessaire.', 'w');
  ...
  const generatePDF = async () => {
    ...
    try {
      // Générer l'ID de vérification et le QR Code AVANT l'aperçu
      const verificationId = generateSecureId();
      setCurrentVerificationId(verificationId);
      const verificationUrl = `${APP_BASE_URL}/verify/${verificationId}`;
      const qrCode = await generateQRCode(verificationUrl);
      ...
      if (savedToSupabase) {
        showNotif("PDF généré et synchronisé !", "s");
      } else {
        showNotif("PDF généré (sauvegarde locale)", "w");
      }
    } catch (error) {
      ...
      const doc = new Document({
        sections: [{
          ...
          children: [
            ...
            new Paragraph({
              children: [new TextRun({ text: `ID de vérification : ${verificationId}`, size: 18 })],
              alignment: AlignmentType.CENTER,
              spacing: { after: 400 }
            }),
            new Paragraph({
            ...
            new Paragraph({
              children: [new TextRun({ text: "Document généré via ECODREUM Engine - Consultez le PDF pour le contrat complet.", italics: true })],
              spacing: { before: 400 }
            }),
            new Paragraph({
              children: [new TextRun({ text: `Vérification : ${APP_BASE_URL}/verify/${verificationId}`, size: 16 })],
              spacing: { before: 200 }
            }),
          ]
        }]
  ...
  const syncToGoogleDrive = () => {
    // Placeholder pour future intégration Google Drive
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 sm:gap-6 mb-6 sm:mb-8">
          <div className="flex items-center gap-3 sm:gap-5">
            ...
            <div>
              ...
              <p className="text-[8px] sm:text-[10px] font-bold text-zinc-500 uppercase tracking-widest mt-1">Générateur de Contrats • ECODREUM Engine V1</p>
            </div>
          </div>
          ...
          <div className="lg:col-span-4">
            <div className="bg-gradient-to-br from-zinc-900 to-zinc-800 border border-white/10 p-4 sm:p-6 md:p-8 rounded-2xl sm:rounded-3xl lg:sticky lg:top-4 space-y-4 sm:space-y-6 shadow-2xl">
              ...
              {currentVerificationId && (
                <div className="p-3 sm:p-4 bg-black/40 rounded-lg sm:rounded-xl border border-white/5">
                  <div className="flex items-center gap-2 text-zinc-400 text-[10px] sm:text-xs font-bold uppercase mb-2">
                    <QrCode size={12} />
                    ID Vérification
                  </div>
                  <p className="text-[9px] sm:text-[10px] text-emerald-400 font-mono break-all">{currentVerificationId}</p>
                </div>
              )}
                ...
                <div className="bg-zinc-900 border border-white/10 rounded-2xl sm:rounded-3xl p-4 sm:p-6 md:p-8">
                  <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6 sm:mb-8">
                    <div>
                      ...
                      <p className="text-[10px] sm:text-xs text-zinc-500 mt-1">Contrats sauvegardés dans le cloud ECODREUM</p>
                    </div>
    ...
    <div className="space-y-4 sm:space-y-6 font-serif text-sm sm:text-base" style={{ fontFamily: 'Georgia, serif', lineHeight: '1.7' }}>
      ...
      <div className="flex items-start justify-between mb-8 sm:mb-12">
        ...
        <div className="text-right text-[8px] sm:text-[10px] text-gray-500">
          <p className="font-mono">{verificationId || 'ID en attente...'}</p>
          <p>ECODREUM Engine V1</p>
        </div>
      </div>
      ...
      <div className="space-y-4 sm:space-y-6 text-xs sm:text-sm">
        ...
        <Article title={`ARTICLE 4 : ${data.jobType === 'STAGE' ? 'GRATIFICATION' : 'RÉMUNÉRATION'}`}>
          <p>
            {data.jobType === 'STAGE' ? 'Le/La Stagiaire' : 'Le/La Salarié(e)'} percevra une {data.jobType === 'STAGE' ? 'gratification' : 'rémunération'} mensuelle {data.jobType === 'STAGE' ? '' : 'brute '}de <strong>{data.salary} {config.currency}</strong> ({salaryToWords(data.salary, config.currency)}).
          </p>
          {data.bonus && <p className="mt-2">Avantages : {data.bonus}.</p>}
        </Article>

        <Article title="ARTICLE 5 : DURÉE DU TRAVAIL">
          <p>La durée hebdomadaire de travail est fixée à <strong>{data.hours} heures</strong>.</p>
        </Article>

        <Article title="ARTICLE 6 : OBLIGATIONS DES PARTIES">
          <p className="font-bold mb-2">6.1. Obligations de l'Employeur :</p>
          <ul className="list-none space-y-1 ml-4 mb-4">
            {config.articles.employerObligations.map((o, i) => (
              <li key={i} className="flex items-start gap-2"><span>•</span><span>{o}</span></li>
            ))}
          </ul>
          <p className="font-bold mb-2">6.2. Obligations du/de la {data.jobType === 'STAGE' ? 'Stagiaire' : 'Salarié(e)'} :</p>
        ...
        <Article title="ARTICLE 7 : CONFIDENTIALITÉ">
          <p>{data.jobType === 'STAGE' ? 'Le/La Stagiaire' : 'Le/La Salarié(e)'} s'engage à observer la plus stricte discrétion sur toutes les informations confidentielles de l'entreprise.</p>
        </Article>

        {data.hasNonCompete && data.jobType !== 'STAGE' && (
          <Article title="ARTICLE 8 : NON-CONCURRENCE">
            <p>Le/La Salarié(e) s'interdit d'exercer toute activité concurrente pendant <strong>{data.nonCompeteDuration}</strong> après la fin du contrat.</p>
          </Article>
        )}
        ...
        <div className="mt-10 sm:mt-16 pt-6 sm:pt-8 border-t-2 border-gray-300 space-y-6 sm:space-y-8">
            ...
            <div className="text-center space-y-3 sm:space-y-4">
              ...
              {data.documentMode === 'ELECTRONIC' && signatures.employer ? (
                <div className="flex flex-col items-center">
                  ...
                  <p className="text-[8px] sm:text-[10px] text-gray-500 mt-1">Signature électronique</p>
                </div>
              ) : (
                <div className="h-16 sm:h-24 border-b-2 border-black flex items-end justify-center pb-1">
                  <p className="text-[8px] sm:text-xs text-gray-400">(Signature)</p>
                </div>
              )}
              <div className="text-[10px] sm:text-sm mt-2">
          ...
          {qrCode && (
            <div className="flex justify-center mt-8 sm:mt-12 pt-6 sm:pt-8 border-t border-gray-200">
              <div className="text-center">
                <img src={qrCode} alt="QR" className="w-20 h-20 sm:w-28 sm:h-28 mx-auto mb-2" />
                <p className="text-[8px] sm:text-[10px] text-gray-500">Scanner pour vérifier l'authenticité</p>
                <p className="text-[7px] sm:text-[9px] text-gray-400 font-mono mt-1">{verificationId}</p>
              </div>
            </div>
          )}
        </div>
        ...
        <div className="mt-8 sm:mt-12 pt-4 sm:pt-6 border-t border-gray-200 text-center text-[8px] sm:text-[10px] text-gray-500">
          <p className="mb-1">Document généré via <strong>ECODREUM Engine V1</strong> - Contrat immuable</p>
