...
const COUNTRIES: Record<'SENEGAL' | 'BURUNDI', CountryConfig> = {
  SENEGAL: {
    ...
    articles: {
      ...
      workDuration: "En application des dispositions relatives à la durée du travail prévues par le Code du Travail sénégalais (articles L.135 et suivants),",
      termination: "Conformément aux dispositions du Code du Travail sénégalais relatives à la rupture du contrat de travail (articles L.48 et suivants),",
      employerObligations: [
      ...
      disputes: "En cas de différend né de l'exécution ou de la rupture du présent contrat, les parties s'engagent à rechercher une solution amiable. À défaut d'accord amiable dans un délai de trente (30) jours, le litige sera soumis au Tribunal du Travail de Dakar, seul compétent pour connaître des litiges individuels du travail, conformément aux dispositions du Code du Travail sénégalais."
    }
  },
  BURUNDI: {
    name: "Burundi",
...
export default function GenerateurContratFinal() {
  ...
  const sendEmail = async () => {
    ...
    try {
      ...
      // Succès !
      showNotif(`✓ Email envoyé avec succès à ${emailRecipient}`, "s");
  ...
  // Calculer la période d'essai selon le type de contrat
  const getTrialPeriod = (): string => {
    if (data.jobType === 'STAGE') return '0';
    if (data.jobType === 'CDD') return '3';
    return data.trial || '3';
  };
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-8">
          <div className="flex items-center gap-5">
            ...
            <div>
              ...
              <p className="text-[10px] font-bold text-zinc-500 uppercase tracking-widest mt-1">Générateur de Contrats • ECODREUM v5.1.0</p>
            </div>
          </div>
              ...
              <div className="bg-zinc-900/50 border border-white/10 p-8 rounded-3xl space-y-6">
                ...
                <div className="bg-amber-500/10 border border-amber-500/30 rounded-xl p-6">
                  ...
                  <InputField 
                    ...
                    placeholder="Décrivez en détail les missions et responsabilités du poste : développement d'applications, gestion de projets, relation client, reporting, etc."
                ...
                {data.jobType === 'CDD' && (
                  <div className="space-y-4">
                    <InputField label="Motif du CDD" value={data.cddReason} onChange={(v) => updateData('cddReason', v)} placeholder="Ex: Remplacement, Accroissement temporaire d'activité..." icon={<FileText size={14} />} required />
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <InputField label="Primes et Avantages" value={data.bonus} onChange={(v) => updateData('bonus', v)} placeholder="Optionnel - Ex: Prime de transport, logement..." icon={<Award size={14} />} />
                </div>
            ...
            <div className="bg-zinc-900 border border-white/10 rounded-3xl p-8 max-w-md w-full">
              ...
              <div className="flex gap-3 mt-6">
                <button 
                  ...
                  className="flex-1 py-3 bg-zinc-800 border border-white/10 text-white rounded-xl font-bold hover:bg-zinc-700 transition-all disabled:opacity-50"
                >
                  Annuler
                </button>
                <button 
...
function ContractPreview({ data, config, signatures, qrCode }: ContractPreviewProps) {
  ...
  // Période d'essai selon le type de contrat
  ...
  return (
    <div className="space-y-6 font-serif" style={{ fontFamily: 'Georgia, serif', lineHeight: '1.8' }}>
      ...
      <div className="text-center mb-12">
        ...
        <p className="text-sm text-gray-500 mt-2">{config.code}</p>
      </div>
      ...
      <div className="space-y-6 text-sm">
        ...
        {/* ARTICLE 8 : CLAUSE DE NON-CONCURRENCE (si applicable) */}
        ...
        <div className="mt-12 pt-6 border-t border-gray-200 text-center text-[10px] text-gray-500">
          {data.documentMode === 'ELECTRONIC' ? (
            <>
              ...
              <p>Ce document a valeur de contrat de travail conformément à la législation en vigueur.</p>
            </>
          ) : (
