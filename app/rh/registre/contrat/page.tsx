      ...
      const doc = new Document({
        sections: [{
          ...
          children: [
            ...
            new Paragraph({
              children: [new TextRun({ text: `Ci-après dénommée « ${data.jobType === 'STAGE' ? "L'ENTREPRISE D'ACCUEIL" : "L'EMPLOYEUR"} »`, italics: true })],
            ...
            new Paragraph({
              children: [new TextRun({ text: `Ci-après dénommé(e) « ${data.jobType === 'STAGE' ? 'LE STAGIAIRE' : 'LE SALARIÉ'} »`, italics: true })],
              alignment: AlignmentType.RIGHT,
              spacing: { after: 400 }
            }),
            new Paragraph({
            ...
            new Paragraph({
              children: [new TextRun({ text: "IL A ÉTÉ ARRÊTÉ ET CONVENU CE QUI SUIT :", bold: true, size: 28 })],
              spacing: { before: 200, after: 200 }
            }),
            new Paragraph({
              children: [new TextRun({ text: "ARTICLE 1 : OBJET ET CADRE LÉGAL", bold: true, size: 24 })],
              spacing: { before: 200, after: 100 }
            }),
            new Paragraph({
              children: [new TextRun({ text: data.jobType === 'STAGE' ? `La présente convention de stage est conclue dans le cadre de la formation de ${data.empName} et s'inscrit dans le parcours pédagogique de l'établissement ${data.stageSchool || '[établissement]'}. Elle est régie par les dispositions légales et réglementaires en vigueur au ${config.name} relatives aux stages en entreprise.` : `Le présent contrat est conclu sous le régime du ${config.code}. ${config.articles.intro} ${config.articles.engagement} Le présent contrat définit les conditions d'engagement et d'emploi du Salarié au sein de la société ${data.compName}.` })],
              spacing: { after: 200 }
            }),
            new Paragraph({
              children: [new TextRun({ text: "ARTICLE 2 : NATURE ET FONCTIONS", bold: true, size: 24 })],
              spacing: { before: 200, after: 100 }
            }),
            new Paragraph({
              children: [new TextRun({ text: `${data.jobType === 'STAGE' ? 'Le Stagiaire' : 'Le Salarié'} est ${data.jobType === 'STAGE' ? 'accueilli' : 'recruté'} en qualité de ${data.jobTitle} au sein du département ${data.jobDept}. ${data.jobType === 'STAGE' ? 'Le Stagiaire' : 'Le Salarié'} exercera ses fonctions au sein de l'établissement situé à ${data.jobLocation}. Le type de ${data.jobType === 'STAGE' ? 'stage' : 'contrat'} conclu est ${data.jobType === 'CDI' ? 'un contrat à durée indéterminée (CDI)' : data.jobType === 'CDD' ? 'un contrat à durée déterminée (CDD)' : 'une convention de stage'}.${data.jobType === 'CDD' && data.cddReason ? ` Le présent contrat est conclu pour les besoins suivants : ${data.cddReason}.` : ''}` })],
              spacing: { after: 200 }
            }),
            new Paragraph({
              children: [new TextRun({ text: `ARTICLE 3 : ${data.jobType === 'STAGE' ? 'GRATIFICATION' : 'RÉMUNÉRATION'}`, bold: true, size: 24 })],
              spacing: { before: 200, after: 100 }
            }),
            new Paragraph({
              children: [new TextRun({ text: `En contrepartie de l'exécution de ses fonctions, ${data.jobType === 'STAGE' ? 'le Stagiaire' : 'le Salarié'} percevra une ${data.jobType === 'STAGE' ? 'gratification' : 'rémunération'} mensuelle ${data.jobType === 'STAGE' ? '' : 'brute '}de ${data.salary} ${config.currency} (${salaryToWords(data.salary, config.currency)}). Cette ${data.jobType === 'STAGE' ? 'gratification est' : 'rémunération est'} versée mensuellement par virement bancaire${data.jobType === 'STAGE' ? '' : ', sous réserve des retenues légales et conventionnelles applicables'}.${data.bonus ? ` En sus de cette rémunération de base, ${data.jobType === 'STAGE' ? 'le stagiaire' : 'le Salarié'} pourra percevoir les primes et avantages suivants : ${data.bonus}.` : ''} ${config.articles.workDuration} la durée hebdomadaire de travail est fixée à ${data.hours} heures.` })],
              spacing: { after: 200 }
            }),
            new Paragraph({
              children: [new TextRun({ text: "ARTICLE 4 : DURÉE ET PÉRIODE D'ESSAI", bold: true, size: 24 })],
              spacing: { before: 200, after: 100 }
            }),
            new Paragraph({
              children: [new TextRun({ text: `${data.jobType === 'STAGE' ? 'La présente convention de stage' : 'Le présent contrat de travail'} prend effet à compter du ${new Date(data.startDate).toLocaleDateString('fr-FR')}${(data.jobType === 'CDD' || data.jobType === 'STAGE') && data.endDate ? ` et prendra fin le ${new Date(data.endDate).toLocaleDateString('fr-FR')}` : ''}.${data.jobType !== 'STAGE' ? ` Une période d'essai de ${data.trial} mois est prévue.` : ''}` })],
              spacing: { after: 200 }
            }),
            ...(data.hasNonCompete && data.jobType !== 'STAGE' ? [
              new Paragraph({
                children: [new TextRun({ text: "ARTICLE 5 : CLAUSE DE NON-CONCURRENCE", bold: true, size: 24 })],
                spacing: { before: 200, after: 100 }
              }),
              new Paragraph({
                children: [new TextRun({ text: `Le Salarié s'engage, pendant une durée de ${data.nonCompeteDuration} suivant la cessation du présent contrat, à ne pas exercer une activité concurrente.` })],
                spacing: { after: 200 }
              })
            ] : []),
            new Paragraph({
            ...
            new Paragraph({
              children: [new TextRun({ text: `Fait à ${data.compAddr.split(',')[0].trim()}, le ${new Date().toLocaleDateString('fr-FR', { day: 'numeric', month: 'long', year: 'numeric' })}` })],
              spacing: { after: 200 }
            }),
            new Paragraph({
              children: [new TextRun({ text: `En deux exemplaires originaux.` })],
              spacing: { after: 400 }
            }),
            new Paragraph({
              children: [new TextRun({ text: data.documentMode === 'ELECTRONIC' ? "Document généré via la plateforme ECODREUM Intelligence." : data.compName, size: 18 })],
              alignment: AlignmentType.CENTER,
              spacing: { before: 400 }
            }),
          ]
      ...
      <div className="max-w-7xl mx-auto p-4 md:p-8">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-8">
          <div className="flex items-center gap-5">
            ...
            <div>
              ...
              <p className="text-[10px] font-bold text-zinc-500 uppercase tracking-widest mt-1">Générateur de Contrats • ECODREUM v5.0.0</p>
            </div>
          </div>
        ...
        <div className="flex gap-3 mb-8 overflow-x-auto pb-2">
          {[
            ...
            <button key={id} onClick={() => setActiveSection(id as any)} className={`flex items-center gap-2 px-8 py-4 rounded-xl text-xs font-black uppercase whitespace-nowrap transition-all ${activeSection === id ? `bg-${color}-500 text-black shadow-lg shadow-${color}-500/30` : 'bg-zinc-800 border border-white/10 text-zinc-400 hover:text-white hover:border-white/20'}`}>
              <Icon size={16} />
              {label}
            </button>
          ))}
          ...
          <div className="lg:col-span-4">
            <div className="bg-gradient-to-br from-zinc-900 to-zinc-800 border border-white/10 p-8 rounded-3xl sticky top-8 space-y-6 shadow-2xl">
              ...
              <div className="space-y-3">
                ...
                {data.empEmail && (
                  <button onClick={() => setShowEmailModal(true)} className="w-full py-4 bg-amber-500/10 hover:bg-amber-500/20 border border-amber-500/30 text-amber-400 rounded-xl font-black uppercase text-xs flex items-center justify-center gap-3 transition-all">
                    <Mail size={18} />
                    Envoyer par Email
                  </button>
                )}
        ...
        {showEmailModal && (
          <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
            <div className="bg-zinc-900 border border-white/10 rounded-3xl p-8 max-w-md w-full">
              ...
              <InputField label="Email destinataire" type="email" value={emailRecipient} onChange={setEmailRecipient} placeholder={data.empEmail || "exemple@email.com"} icon={<Mail size={14} />} />
              <div className="flex gap-3 mt-6">
                <button onClick={() => setShowEmailModal(false)} className="flex-1 py-3 bg-zinc-800 border border-white/10 text-white rounded-xl font-bold hover:bg-zinc-700 transition-all">Annuler</button>
                <button onClick={sendEmail} className="flex-1 py-3 bg-amber-500 text-black rounded-xl font-bold hover:bg-amber-400 transition-all">Envoyer</button>
              </div>
            </div>
          </div>
        )}
                    ...
                    {filteredContracts.map((contract) => (
                      <div key={contract.id} className="bg-zinc-800 border border-white/10 rounded-xl p-6 space-y-3">
                        ...
                        <div className="flex gap-2 pt-3">
                          <button onClick={() => loadContract(contract)} className="flex-1 py-2 bg-blue-500/20 border border-blue-500/30 text-blue-400 rounded-lg text-xs font-bold hover:bg-blue-500/30 transition-all">Charger</button>
                          <button onClick={() => deleteContract(contract.id)} className="p-2 bg-red-500/20 border border-red-500/30 text-red-400 rounded-lg hover:bg-red-500/30 transition-all"><Trash2 size={14} /></button>
                        </div>
                      </div>
                    ))}
                  </div>
  ...
  return (
    <div className="space-y-6 font-serif" style={{ fontFamily: 'Georgia, serif', lineHeight: '1.8' }}>
      ...
      <div className="space-y-6 text-sm">
        ...
        <p className="leading-relaxed">La société <strong>{data.compName}</strong>, {data.compType}{capitalClause}, dont le siège social est situé à <strong>{data.compAddr}</strong>, immatriculée au Registre de Commerce et du Crédit Mobilier (RCCM) sous le numéro <strong>{data.compRCCM}</strong> et identifiée au {config.idLabel} sous le numéro <strong>{data.compID}</strong>, représentée par M./Mme <strong>{data.bossName}</strong> en sa qualité de <strong>{data.bossTitle}</strong>, dûment habilité(e) aux fins des présentes.</p>
        ...
        <p className="leading-relaxed">M./Mme <strong>{data.empName}</strong>, né(e) le <strong>{data.empBirth ? new Date(data.empBirth).toLocaleDateString('fr-FR') : '___'}</strong> à <strong>{data.empBirthPlace}</strong>, de nationalité <strong>{data.empNation}</strong>{foreignerClause}, titulaire de la pièce d'identité n°<strong>{data.empID}</strong>, demeurant à <strong>{data.empAddr}</strong>, joignable au <strong>{data.empPhone}</strong>{data.empEmail && ` et par email à ${data.empEmail}`}.{data.jobType === 'STAGE' && data.stageSchool && (<> Actuellement inscrit(e) en <strong>{data.stageLevel}</strong> à <strong>{data.stageSchool}</strong>.</>)}</p>
        ...
        <Article title="ARTICLE 1 : OBJET ET CADRE LÉGAL">
          {data.jobType === 'STAGE' ? (
            <>La présente convention de stage est conclue dans le cadre de la formation de {data.empName} et s'inscrit dans le parcours pédagogique de l'établissement {data.stageSchool}. Elle est régie par les dispositions légales et réglementaires en vigueur au {config.name} relatives aux stages en entreprise.</>
          ) : (
        ...
        <Article title="ARTICLE 2 : NATURE ET FONCTIONS">
          {data.jobType === 'STAGE' ? 'Le Stagiaire' : 'Le Salarié'} est {data.jobType === 'STAGE' ? 'accueilli' : 'recruté'} en qualité de <strong>{data.jobTitle}</strong> au sein du département <strong>{data.jobDept}</strong>. {data.jobType === 'STAGE' ? 'Le Stagiaire' : 'Le Salarié'} exercera ses fonctions au sein de l'établissement situé à <strong>{data.jobLocation}</strong>. Le type de {data.jobType === 'STAGE' ? 'stage' : 'contrat'} conclu est {data.jobType === 'CDI' ? 'un contrat à durée indéterminée (CDI)' : data.jobType === 'CDD' ? 'un contrat à durée déterminée (CDD)' : 'une convention de stage'}.{data.jobType === 'CDD' && data.cddReason && ` Le présent contrat est conclu pour les besoins suivants : ${data.cddReason}.`}
        </Article>

        <Article title={`ARTICLE 3 : ${data.jobType === 'STAGE' ? 'GRATIFICATION' : 'RÉMUNÉRATION'}`}>
          En contrepartie de l'exécution de ses fonctions, {data.jobType === 'STAGE' ? 'le Stagiaire' : 'le Salarié'} percevra une {data.jobType === 'STAGE' ? 'gratification' : 'rémunération'} mensuelle {data.jobType === 'STAGE' ? '' : 'brute '}de <strong>{data.salary} {config.currency}</strong> ({salaryToWords(data.salary, config.currency)}). Cette {data.jobType === 'STAGE' ? 'gratification est' : 'rémunération est'} versée mensuellement par virement bancaire{data.jobType === 'STAGE' ? '' : ', sous réserve des retenues légales et conventionnelles applicables'}.{data.bonus && ` En sus de cette rémunération de base, ${data.jobType === 'STAGE' ? 'le stagiaire' : 'le Salarié'} pourra percevoir les primes et avantages suivants : ${data.bonus}.`} {config.articles.workDuration} la durée hebdomadaire de travail est fixée à <strong>{data.hours} heures</strong>.
        </Article>

        <Article title="ARTICLE 4 : DURÉE">
          {data.jobType === 'STAGE' ? 'La présente convention de stage' : 'Le présent contrat de travail'} prend effet à compter du <strong>{data.startDate ? new Date(data.startDate).toLocaleDateString('fr-FR') : '___'}</strong>{endDateClause}.{data.jobType !== 'STAGE' && (<> Une période d'essai de <strong>{data.trial} mois</strong> est prévue.</>)}
        </Article>
        ...
        <div className="mt-16 pt-8 space-y-8">
          <p className="text-sm">Fait à <strong>{data.compAddr.split(',')[0].trim()}</strong>, le <strong>{new Date().toLocaleDateString('fr-FR', { day: 'numeric', month: 'long', year: 'numeric' })}</strong></p>
...
function InputField({ label, value, onChange, type = "text", placeholder = "...", icon, required = false, disabled = false, multiline = false }: InputFieldProps) {
  return (
    <div className="flex flex-col gap-2">
      ...
      {multiline ? (
        <textarea value={value} onChange={(e) => onChange(e.target.value)} placeholder={placeholder} disabled={disabled} rows={3} className="bg-black/40 border border-white/10 rounded-xl p-3 text-sm text-white outline-none focus:border-emerald-500 focus:bg-emerald-500/5 transition-all disabled:opacity-40 disabled:cursor-not-allowed resize-none" />
      ) : (
        <input type={type} value={value} onChange={(e) => onChange(e.target.value)} placeholder={placeholder} disabled={disabled} className="bg-black/40 border border-white/10 rounded-xl p-3 text-sm text-white outline-none focus:border-emerald-500 focus:bg-emerald-500/5 transition-all disabled:opacity-40 disabled:cursor-not-allowed" />
      )}
    </div>
  );
}
